...
#endif
{
	...
	while ( true )
	{
		...
		if ( !bVRR )
		{
			...
			if (sleep_cycle > 1)
			{	
				long double roll = .98*std::exp(-((.98*static_cast<long double>(rollingMaxDrawTime)/std::sqrt(vblank_adj_factor)-.9*static_cast<long double>(redZone)*vblank_adj_factor)/10000000.0));
				...
				g_uRollingMaxDrawTime = rollingMaxDrawTime = (static_cast<uint64_t>(llroundl(roll)))+redZone;
			}
			else
			{
