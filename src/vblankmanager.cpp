...
#endif
{
	...
	while ( true )
	{
		...
		if ( !bVRR )
		{
			...
			if (sleep_cycle > 1)
			{	
				long double roll = std::exp(-(.5*(static_cast<long double>(rollingMaxDrawTime)/std::sqrt(vblank_adj_factor)-static_cast<long double>(redZone)*vblank_adj_factor)/10000000.0));
