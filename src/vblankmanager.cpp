...
#endif
{
	...
	while ( true )
	{
		...
		if ( !bVRR )
		{
			...
			if (sleep_cycle > 1)
			{	
				long double roll = std::exp(-(.5*(std::sqrt(vblank_adj_factor)*static_cast<long double>(rollingMaxDrawTime)-static_cast<long double>(redZone)*2.5/vblank_adj_factor)/10000000.0));
