...
#endif
{
	...
	while ( true )
	{
		...
		if ( !bVRR )
		{
			...
			if (sleep_cycle > 1)
			{	
				long double roll = .95*std::exp(-((.8*static_cast<long double>(rollingMaxDrawTime)/std::sqrt(vblank_adj_factor)-.9*static_cast<long double>(redZone)*vblank_adj_factor)/10000000.0));
