...
#endif
{
	...
	while ( true )
	{
		...
		if ( !bVRR )
		{
			...
			if (sleep_cycle > 1)
			{	
				long double roll = .98*std::exp(-((.85*std::pow(static_cast<long double>(rollingMaxDrawTime),.666666666666666666)*std::sqrt(vblank_adj_factor)-.9*static_cast<long double>(redZone)*vblank_adj_factor)/10000000.0));
