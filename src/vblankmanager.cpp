...
#endif
{
	...
	while ( true )
	{
		...
		if ( !bVRR )
		{
			...
			{
				rollingMaxDrawTime = std::clamp(centered_mean*2 - std::abs(static_cast<int64_t>(lastDrawTime) - static_cast<int64_t>(drawTime)), ( ( alpha * rollingMaxDrawTime ) + ( range - alpha ) * drawTime ) / (range), static_cast<uint64_t>(llroundl( static_cast<double>((centered_mean/2)) * std::pow(static_cast<double>(drawTime)/(std::max(static_cast<double>( std::abs(static_cast<int64_t>(lastDrawTime) - static_cast<int64_t>(drawTime))), 1.0 )), 2))));
			}
			...
			if ( sleep_cycle < 2 && index >= 64 )
			{
				...
				max_drawtime = std::min( 
					      ...
					      * 500)
					      <<1
					, 8*nsecInterval/3);
				time_discount=readCycleCount() - time_discount;
			}
...
# endif	
		...
		else
		{
			...
			targetPoint = vblank_next_target(  static_cast<uint64_t>( llroundl( static_cast<long double>((offset*refresh * sleep_weights[sleep_cycle-1] ) / static_cast<long double>(100*g_nOutputRefresh)) - static_cast<long double>(time_discount)*g_nsPerTick )));
